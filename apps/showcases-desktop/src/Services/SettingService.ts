...
export function SettingsService(filesService: TFilesService): TSettingsService {
  ...
  const loadAppSettings = async (): Promise<TShowcaseGameSettings | undefined> => filesService.readJsonFile(userDataFolder, isSettings);
  ...
  async function saveAppSettings(settings: TShowcaseGameSettings): Promise<void> {
    ...
    await filesService.writeFile(userDataFolder, appSettingsFileName, settings);
