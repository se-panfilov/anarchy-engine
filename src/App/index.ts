...
// import type { IVector2Wrapper, ICameraWrapper, ILaunchedEngine, ILaunchedScene, IMousePosition, IRegistries, IVector3 } from '@/Engine';
...
import type { ILaunchedEngine } from '@/Engine';
import { launchEngine, SceneLauncher } from '@/Engine';

const { factories, canvas }: ILaunchedEngine = launchEngine('#app');
// const { registries }: ILaunchedScene = SceneLauncher().launch(sceneConfig, canvas, factories);
SceneLauncher().launch(sceneConfig, canvas, factories);
// const { actorRegistry, cameraRegistry }: IRegistries = registries;
