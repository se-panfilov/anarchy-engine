...
import type { TActor, TActorWithPhysics, TAppCanvas, TCameraWrapper, TEngine, TSpace, TSpaceConfig } from '@/Engine';
...
export async function showcase(canvas: TAppCanvas): Promise<TShowcase> {
  ...
  function init(): void {
    const actor1W: TActorWithPhysics | TActor | undefined = actorAsyncRegistry.findByName('actor_1');
    if (isNotDefined(actor1W)) throw new Error(`Cannot find "actor_1" actor`);

    const actor2W: TActorWithPhysics | TActor | undefined = actorAsyncRegistry.findByName('actor_2');
    if (isNotDefined(actor2W)) throw new Error(`Cannot find "actor_2" actor`);

    const actor3W: TActorWithPhysics | TActor | undefined = actorAsyncRegistry.findByName('actor_3');
