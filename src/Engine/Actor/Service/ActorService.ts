...
export function ActorService(factory: IActorFactory, registry: IActorAsyncRegistry, materialService: IMaterialService, scene: ISceneWrapper): IActorService {
  ...
  const createAsync = (params: IActorParams): Promise<IActorWrapperAsync> => factory.createAsync(params, materialService);
  const createFromConfig = (actors: ReadonlyArray<IActorConfig>): void => {
    // eslint-disable-next-line @typescript-eslint/no-misused-promises
    actors.forEach((config: IActorConfig): Promise<IActorWrapperAsync> => factory.createAsync(factory.configToParams(config), { materialService }));
  };
