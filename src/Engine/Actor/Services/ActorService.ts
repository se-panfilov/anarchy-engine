...
export function ActorService(factory: TActorFactory, registry: TActorAsyncRegistry, actorDependencies: TActorServiceDependencies, scene: TSceneWrapper): TActorService {
  ...
  const createAsync = (params: TActorParams): Promise<TActorWrapperAsync | TActorWrapperWithPhysicsAsync> => factory.createAsync(params, actorDependencies);
  const createFromConfig = (actors: ReadonlyArray<TActorConfig>): void => {
    // eslint-disable-next-line @typescript-eslint/no-misused-promises
    actors.forEach((config: TActorConfig): Promise<TActorWrapperAsync | TActorWrapperWithPhysicsAsync> => factory.createAsync(factory.configToParams(config), actorDependencies));
  };
