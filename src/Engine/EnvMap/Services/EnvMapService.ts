...
import type { TDataTexture, TEnvMapAsyncRegistry, TEnvMapService } from '@/Engine/EnvMap/Models';
...
export function EnvMapService(registry: TEnvMapAsyncRegistry): TEnvMapService {
  ...
  added$.subscribe((texture: TDataTexture): void => registry.add(texture));
  ...
  function loadAsync(url: string, isForce: boolean = false): Promise<TDataTexture> {
    if (!isForce) {
      const texture: TDataTexture | undefined = registry.getByKey(url);
