...
export function EnvMapLoader(registry: TEnvMapTextureAsyncRegistry): TEnvMapLoader {
  ...
  function loadAsync({ url, isForce, name, mapping }: TEnvMapResourceConfig): Promise<TEnvMapTexture> {
    if (!isForce) {
    ...
    return envMapLoader.loadAsync(url).then((envMap: TWriteable<TEnvMapTexture>): TEnvMapTexture => {
      ...
      envMap.mapping = isDefined(mapping) ? mapping : EquirectangularReflectionMapping;
