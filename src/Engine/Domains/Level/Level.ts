...
import type { IText2dRenderer, IText2dWrapper, IText3dRenderer, IText3dWrapper, ITextConfig, ITextFactory, ITextRegistry } from '@/Engine/Domains/Text';
...
export function buildLevelFromConfig(canvas: IAppCanvas, config: ILevelConfig): ILevel {
  ...
  const textAddedSubscription: Subscription = textRegistry.added$.subscribe((text: IText2dWrapper | IText3dWrapper) => scene.addText(text));
  const textEntityCreatedSubscription: Subscription = textFactory.entityCreated$.subscribe((text: IText2dWrapper | IText3dWrapper): void => textRegistry.add(text));
  texts.forEach((text: ITextConfig): IText2dWrapper | IText3dWrapper => textFactory.create(textFactory.configToParams({ ...text, tags: [...text.tags, CommonTag.FromConfig] })));
