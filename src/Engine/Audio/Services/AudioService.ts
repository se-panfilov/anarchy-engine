...
export function AudioService(factory: TAudioFactory, registry: TAudioRegistry, audioResourceAsyncRegistry: TAudioResourceAsyncRegistry, { audioLoop }: TSpaceLoops): TAudioService {
  ...
  const createFromConfig = (models3d: ReadonlyArray<TAudioConfig>): ReadonlyArray<TAudioWrapper> =>
    models3d.map((config: TAudioConfig): TAudioWrapper => create(factory.configToParams(config, { audioResourceAsyncRegistry })));
