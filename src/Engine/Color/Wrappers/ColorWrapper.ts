...
import { entityToConfig } from '@/Engine/Color/Adapters';
...
export function ColorWrapper(color: TColorParams): TColorWrapper {
  ...
  const result = Object.assign(AbstractWrapper(entity, WrapperType.Color), { entity, serialize: (): ColorRepresentation => entityToConfig(result) });
  return result;
}
