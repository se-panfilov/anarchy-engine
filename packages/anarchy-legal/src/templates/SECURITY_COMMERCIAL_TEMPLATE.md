# Security Policy — [GAME NAME]

**Effective date:** [YYYY-MM-DD]
**Contact:** security@[YOUR-DOMAIN] (PGP below)

## Scope

This policy applies to commercial builds of **[GAME NAME]** distributed via **[STEAM / …]**. It describes how to contact us about potential security issues. **It does not create service levels, warranties, or contractual obligations.**

## Reporting a Vulnerability

Email **security@[YOUR-DOMAIN]**. Please include the affected version/platform, minimal reproduction, impact, and (if available) a small PoC.
**Encryption is not required.** If you prefer encrypted exchange or need to share large files, contact us.

## Handling

We will make a **good-faith effort** to review and prioritize reports. **No specific response times or SLAs are promised.** Availability may be reduced during weekends, holidays, or maintainer absences.

## Regulatory Notifications

Where required by law, we will **notify competent authorities and/or users within the legally mandated timelines** for actively exploited vulnerabilities or other reportable events.

## Updates & Delivery

Security updates are delivered through **[STORE AUTO-UPDATES]**. Release notes and advisories (when applicable) are posted at **[URL]**.

## In/Out of Scope

- In scope: security issues that materially impact the confidentiality, integrity, or availability of the shipped game or its update mechanism.
- Out of scope: social engineering, physical attacks, denial-of-service on third-party platforms, vulnerabilities in third-party stores or OS kernels not packaged with the game.

## Safe Harbor

If you comply with this policy and act in good faith, we **do not intend** to pursue legal action solely for your research on our game. This policy does not authorize any activity that would violate applicable law or third-party terms.
