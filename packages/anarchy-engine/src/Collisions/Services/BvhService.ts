...
import type { TBvhOptions, TBvhService } from '@hellpig/anarchy-engine/Collisions/Models';
...
export function BvhService(): TBvhService {
  ...
  function createBvhForActor(actor: TActor, options?: TBvhOptions): void {
    processEntity(actor.model3d.getRawModel3d(), (mesh: Mesh) => computeBVHBoundsTree(mesh.geometry, options));
  }
  ...
  function _debugVisualizeBvhForActor(actor: TActor, sceneW: TSceneWrapper, depth: number = 10): void {
    processEntity(actor.model3d.getRawModel3d(), (mesh: Mesh) => {
